  ...
  def render(assigns) do
    ...
    <div class="relative">
      <.hero_banner image={@zone_image} height={:short}>
        ...
        >
          ← {if @zone, do: @zone.name, else: "Map"}
        </a>
        <div class="flex items-center gap-2 mb-1">
          <.quest_type_badge type={@quest.type} />
          <span class="text-xs text-slate-400">Quest {@quest.number}</span>
        </div>
        <div class="flex items-center justify-between">
          <h1 class="text-2xl md:text-3xl font-black text-slate-100">{@quest.name}</h1>
          <div class={[
            "text-xl font-black",
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
      <%!-- NPC Greeting --%>
      <%= if @quest.npc_greeting && @zone && @zone.npc && @npc_portrait do %>
        <div class="mb-6">
          <.npc_dialogue_card npc_name={@zone.npc} portrait_url={@npc_portrait} zone_number={@zone_number}>
            "{@quest.npc_greeting}"
          </.npc_dialogue_card>
        </div>
      <% end %>
      ...
      <div class={[
        ...
        <%= if @completed && @quest.npc_confirmation && @zone && @zone.npc && @npc_portrait do %>
          <div class="mb-6">
            <.npc_dialogue_card npc_name={@zone.npc} portrait_url={@npc_portrait} zone_number={@zone_number}>
              "{@quest.npc_confirmation}"
            </.npc_dialogue_card>
          </div>
        <% else %>
          <div class={[
